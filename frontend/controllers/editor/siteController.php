...
use alimmvc\frontend\models\saveSite;
...
class siteController extends authController
{
	...
	public function actionEdit($id_site)
	{
		...
		foreach ($styles as $value) {
			if (isset($styles2[$value['id_tag']])) 
			{
				$styles2[$value['id_tag']]['selector'] .= "{$value['selector']}{{$value['style']}}";
			}
			else {
				$styles2[$value['id_tag']]['id_tag'] = $value['id_tag'];
				$styles2[$value['id_tag']]['selector'] = "{$value['selector']}{{$value['style']}}";
			}
		}
	...
	public function actionAddHtml($id_site, $nameTemplate, $parent_tag = 0, $index_tag = 0)
	{
		...
		if (!empty($tagsData))
		{
			...
			// !!!!!!!! баг (parent_tag не правильно), когда большая вложенность в шаблоне
			// добавим в бд
			foreach ($tags as $data)
			{
				// debug($data);
				$tags[$data['id_tag']]['id_tag'] = $first_id_tag++;
